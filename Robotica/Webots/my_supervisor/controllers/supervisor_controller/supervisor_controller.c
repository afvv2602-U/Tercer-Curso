...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (i > 20) {
      for (int j = 0; j < 100; j++) {
        char bb8_instance[128];
        sprintf(bb8_instance, "BB-8 { translation %f %f %f }", 1.0 * j, 1.0 * j, 1.0 * j); 

        wb_supervisor_field_import_mf_node_from_string(children_field, -1, bb8_instance);
    }

      WbNodeRef floor_node = wb_supervisor_node_get_from_def("floor");
      WbFieldRef floor_scale_field = wb_supervisor_node_get_field(floor_node, "floorSize");
      const double new_scale[2] = {1000, 1000};
      wb_supervisor_field_set_sf_vec2f(floor_scale_field, new_scale);
